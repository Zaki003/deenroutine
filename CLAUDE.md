...
- Everything except the daily quote's and the habit reminders' *existing* opt-out is off by default (onboarding promises reminders "only for the habits you choose"). Encouraging reminder wording is on by default but can be switched back to the plain "Time for: …" line.
