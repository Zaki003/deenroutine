...
`test/widget_test.dart` is still the default `flutter create` counter-app boilerplate — it doesn't exercise this app and will fail if run as-is. The only real tests are `test/habit_insights_test.dart`, `test/prayer_waqt_test.dart`, `test/prayer_stats_test.dart` and `test/prayer_check_in_plan_test.dart`, covering the Profile's habit stats and prayer tracking's waqt rules, stats and check-in planning.
...
- The Profile's "Prayer insights" section (`PrayerStatsSection`, maths in `computePrayerStats`) is collapsed by default for privacy, with the open state remembered per device in SharedPreferences, and it fetches nothing until it's opened. Once open, it loads the last 90 days once per prayer day via `PrayerLogProvider.loadHistory` and merges in today's live record, so logging a prayer updates it without another fetch. Days before the first logged one are ignored rather than counted as missed. The streak can't see past the 90 days loaded, so it shows as "N+" when it reaches that far.
...
**Habit insights** (Profile, `HabitInsightsSection`, maths in `computeHabitInsights` in `lib/utils/habit_insights.dart`): best current streak, a 4-week rate, total check-ins, a 4-week Mon-Sun calendar grid, this week's bars (tap a day for what was done), and 7/30/40/100-day milestone badges. Open by default and collapsible like Prayer insights, fetching nothing while closed. Things that aren't obvious:
- `HabitProvider.loadInsightsHistory` fetches each recurring habit's *whole* `HabitLogs` history (the same per-habit query streaks already use; no new index) once per day, keeping only days before today; `HabitProvider.insights` rebuilds today's log from the live habit row (`hasProgressToday` means a log exists, `completed` is its status, and an avoidance slip is a status-false log), so ticking a habit updates the section without another fetch. A habit added later only fetches its own history.
- Milestones use each habit's **longest-ever** streak, not its current one, so a badge stays earned after a streak breaks. They're separate from `kMilestoneDays` (the dashboard's celebration banners).
- The 4-week rate leaves today out (still in progress); the grid and this week's bars include it. Schedule-aware like the streaks: a `specificDays` habit is only due on its days, and nothing is due before a habit's `createdAt`. Its load error lives in `insightsErrorType`, apart from `errorType`, which the dashboard shows as a snackbar.
- Days with every due habit done get a star in the grid rather than a "perfect days" count in the numbers row: with several habits that count usually sits at 0, which reads as failure, so it's celebrated when it happens instead of scored.
- A longer grid (13 weeks) is meant for the paid tier later - `kInsightGridWeeks`.
